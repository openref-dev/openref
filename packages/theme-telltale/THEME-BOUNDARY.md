...
## 1. 107 class names the theme did not write, and cannot replace
...
**AND THE PIN NOW COVERS FOUR FIGURES OF THIS SECTION RATHER THAN ONE, since the post-`T054` review.
It found the other three stale.** The heading moved to 99 and the sentence above still said twelve
renders, the styled-of-total sentence below still said 85 of 86, and the arrival table still summed
to 86 with no row for the thirteen names `T054` added. One figure had a runner and three did not,
which is the same defect the runner exists against, one level down. The case in
`theme-boundary.spec.ts` now reads all four off the measurement: the count, the render total, the
styled total, and the sum of the deltas in the table below.
...
of what a frozen contract is for. 106 of the 107 are styled here; the one that is not,
