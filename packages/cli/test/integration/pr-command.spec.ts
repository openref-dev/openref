...
describe('openref pr against a real repository', () => {
  ...
  it('should report an added operation as a non breaking change and exit 0', async () => {
  ...
  it('should report a removed operation as breaking, and exit 1 only when asked to', async () => {
  ...
  it('should report a changed schema type in the block that is not the routes', async () => {
  ...
  it('should create the comment once and update it in place on the next push', async () => {
  ...
  it('should send no request at all from a fork, print the comment, and exit 0', async () => {
  ...
  it('should fail as a usage error when the base ref is not in the repository', async () => {
    // Given
...
describe('the repository the comment is addressed to', () => {
  ...
  it('should reach exactly /repos/<owner>/<name>/issues/<n>/comments for a legal value', async () => {
  ...
  it('should refuse the same spellings given as a flag, which is the other way in', async () => {
    // Given
    repo = await makeRepo(withoutDeleteOperation());
...
describe('the API root the token is sent to', () => {
  ...
  it('should reach a loopback root over http, which is where the fake listens', async () => {
  ...
  it('should reach the same fake spelled localhost, which is the other way a person writes it', async () => {
    // Given
    repo = await makeRepo(withoutDeleteOperation());
...
describe('whose comment may be overwritten', () => {
  ...
  it('should adopt and patch the comment its own identity wrote', async () => {
  ...
  it('should leave a contributor comment alone even when its first line is the marker', async () => {
  ...
  it('should keep one comment under an installation token, by the app it acted as', async () => {
  ...
  it('should post and say so when neither identity path answers', async () => {
...
describe('a fork status this run cannot establish', () => {
  ...
  it('should refuse with exit 2 when the named event payload cannot be read', async () => {
  ...
  it('should still refuse when the number and repository were supplied by flags', async () => {
  ...
  it('should print and exit 0 on a dry run, since a dry run posts nothing anyway', async () => {
...
describe('the step outputs a workflow reads', () => {
  ...
  it('should write one line per output, which is the shape the reader parses', async () => {
  ...
  it('should withhold an output whose value carries a newline, and say so', async () => {
  ...
  it('should withhold it the same way when the value arrived by the environment', async () => {
...
describe('a redirect answered to a request carrying the token', () => {
  ...
  it('should flow normally against a server that answers rather than redirects', async () => {
  ...
  it('should refuse a same origin redirect, which is the one that keeps the header', async () => {
  ...
  it('should refuse a cross origin redirect too, and reach the other origin not at all', async () => {
    // Given a second server on another origin, and a first that points every request at it
    elsewhere = await startFakeGitHub();
    github = await startFakeGitHub({ redirectTo: `${elsewhere.url}/repos/a/b/issues/1/comments` });
    repo = await makeRepo(withoutDeleteOperation());
    const env = await postingEnvironment(github.url);
...
describe('a thread longer than the search is allowed to read', () => {
  ...
  it('should say the cap was reached rather than post a second comment in silence', async () => {
  ...
  it('should say nothing about a cap on a thread that simply ended', async () => {
