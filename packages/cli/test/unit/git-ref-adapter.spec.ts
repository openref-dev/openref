...
describe('readGitBlob', () => {
  it('should read a committed file at HEAD', async () => {
  ...
  it('should refuse a ref that starts with a hyphen before spawning git', async () => {
    // When
  ...
  it('should refuse a path that starts with a hyphen', async () => {
    // When / Then
    await expect(readGitBlob('HEAD', '-o/tmp/x')).rejects.toThrow('git would read as an option');
  });

  it('should report a ref that does not resolve as a usage error naming both sides', async () => {
    // When
  ...
  it('should report a file that is not in the revision rather than answer an empty document', async () => {
    // When / Then
...
describe('loadGitDocument', () => {
  it('should normalize what git produced, and close without holding anything', async () => {
