 ...
 * and the case that proves a body-carrying one does are checked against the same fifteen entries.
...
const BODY_MARKERS: Readonly<Record<SampleLanguageId, readonly string[]>> = {
  ...
  ruby: ['request.body ='],
  rust: ['.body('],
  swift: ['request.httpBody'],
  kotlin: ['.toRequestBody('],
  dart: ['request.bodyBytes'],
};

describe('a sample for an operation with no body', () => {
  it('should emit no body argument in any of the fifteen languages', () => {
    // Given
  ...
  it('should say the request has no body where the language insists on saying something', () => {
    // Given, the three clients whose call takes a body argument that cannot be left out
    ...
    expect(byLang.get('kotlin')).toContain('.method("GET", null)');
  });
