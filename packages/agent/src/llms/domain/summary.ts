 ...
 *
 * IT LIVES IN `@openref/core` SINCE `T062`, BESIDE `plainArtefactText`, WHOSE PER VALUE HALF IT IS.
 * The ruling of SPEC 16.1 gave the static build's `llms.txt` the same rule, and `@openref/static`
 * may not see this package; a second spelling there would be the duplication SPEC 18.1's own
 * paragraph about two generators exists against. The name is re-exported so every caller and every
 * case that named it here still does.
 */
export { oneLine } from '@openref/core';
